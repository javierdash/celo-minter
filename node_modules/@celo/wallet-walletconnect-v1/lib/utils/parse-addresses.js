...
function parseShortNameAddress(addressLike) {
    ...
    if (!celo ||
        !address ||
        invalidChain(celo) ||
        !utils_1.AddressUtils.isValidAddress(address)) {
...
function parseCaip50Address(addressLike) {
    ...
    if (!chain ||
        ...
        !utils_1.AddressUtils.isValidAddress(address) ||
        invalidChain(chain)) {
        throw new Error("Invalid CAIP50 address ".concat(address || '<undefined address>'));
    }
...
function parseCaip10Address(addressLike) {
    ...
    if (!address ||
        ...
        !utils_1.AddressUtils.isValidAddress(address) ||
        invalidChain(chain)) {
        throw new Error("Invalid CAIP10 address ".concat(address || '<undefined address>'));
    }
    return { address: address, networkId: networkId };
}
